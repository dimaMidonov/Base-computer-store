...
class RouterManager:
    def __init__(self, database_model: Type[BaseModel], pydantic_model: Type[BaseModelModify]) -> None:
        ...
        self.fastapi_router: fastapi.APIRouter = fastapi.APIRouter(
            prefix=self.camel_case_to_url(),
            tags=[pydantic_model.__name__]
        )
        self.resolver_manager = ResolverManager(self.database_model, self.pydantic_model)
        self.init_routers()

    def camel_case_to_url(self):
        result: str = ''

        for word in re.findall(r'[A-Z][a-z]*', self.pydantic_model.__name__):
            result += word.lower() + '_'
